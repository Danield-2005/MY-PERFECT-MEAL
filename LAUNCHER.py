...
GOOGLE_MAPS_KEY = "AIzaSyDAAxC_kLdAEqCppRF4HhssFQUSnf51ltM"  # Replace with your API key
...
# Move haversine function to global scope so it can be used by multiple functions
    ...
    try:
        ...
        for place in places_result.get('results', []):  # Iterate through API results
            place_details = gmaps.place(  # Fetch detailed info for each place
                place['place_id'],
                fields=['name', 'rating', 'formatted_address', 'formatted_phone_number', 'price_level', 'reviews', 'opening_hours', 'website', ]
            )['result']
            ...
            distance_km = haversine(lat, lon, place['geometry']['location']['lat'], place['geometry']['location']['lng'])
            ...
            restaurant = {
                ...
                'is_open_now': place_details.get('opening_hours', {}).get('open_now', None),
...
def surprise_me_recommendation(user_id, location):
    ...
    try:
        ...
        if rf_model is None:
            st.error("Machine Learning model is not loaded. Please check if 'random_forest_model.pkl' exists.")
            return []
            
        # Fetch user and recent search data
        user_info = get_user_data(user_id)
        if not user_info:
            st.warning("No user data found. Make sure you complete the profile setup.")
            return []
            
        recent_search = get_recent_search(user_id)
        if not recent_search:
            st.warning("No recent search history found. Please perform a search first.")
            return []

        # Prepare the feature vector with corrected function
        feature_vector = prepare_feature_vector(user_info, recent_search)
        
        # Log for debugging
        print(f"Feature vector for prediction: {feature_vector}")
        
        # Debug monitoring of the prediction
        try:
            prediction = rf_model.predict([feature_vector])[0]
            st.info(f"Model prediction: {'Recommend' if prediction == 1 else 'Do not recommend'}")
        ...
        if not coords:
            st.warning("Unable to determine coordinates for the specified location.")
            return []
        ...
        # Loop through each place returned from the API,
        # fetch detailed information, compute distance, and format the data
        # Parse results
        results = []
        for place in places_result.get('results', []): 
            try:
                # Get detailed information for each place using Google Maps Place Details API
                place_details = gmaps.place(
                    place['place_id'], 
                    fields=['name', 'rating', 'formatted_address', 'formatted_phone_number', 
                           'price_level', 'reviews', 'website', 'opening_hours']
                )['result']
                
                # Convert price level to dollar signs ('$'), if available
                price_level = place_details.get('price_level', 0)
                price = '$' * (price_level + 1) if price_level is not None else 'N/A'
                reviews = place_details.get('reviews', [])
                latest_reviews = reviews[:3] if reviews else []  # Retrieve up to 3 most recent reviews

                # Calculate distance from user to the restaurant using the haversine formula
                distance_km = haversine(lat, lon, 
                                       place['geometry']['location']['lat'], 
                                       place['geometry']['location']['lng'])
     
                # Build a structured dictionary of place information
                results.append({
                    'name': place_details.get('name', 'N/A'),
                    'cuisine': ', '.join(keywords) if keywords and keywords[0] != "restaurant" else 'Various',
                    'rating': place_details.get('rating', 'N/A'),
                    'price': price,
                    'address': place_details.get('formatted_address', 'N/A'),
                    'phone': place_details.get('formatted_phone_number', 'N/A'),
            ...
            except Exception as place_error:  # Handle errors that may occur while processing individual places
                print(f"Error processing place {place.get('name', 'unknown')}: {str(place_error)}")
                continue

        return results
    except Exception as e:  # Handle unexpected errors in the recommendation process
...
def main():
    ...
    # "Find Restaurants" button first (moved up)
    if st.button("Find Restaurants"):  # "Find Restaurants" button
        if location:  # Ensure location is provided
            ...
            if results:
                ...
                for business in results:  # Loop through each recommended restaurant and display its information
                    st.write(f"{business['name']}")
                    st.write(f"Rating: {'⭐' * round(float(business['rating'])) if isinstance(business['rating'], (int, float)) else 'N/A'}")
                    st.write(f"💵Price: {business.get('price', 'N/A')}")  # Show restaurant price
                    st.write(f"🏠︎Address: {business.get('address', 'N/A')}")  # Show restaurant address
                    st.write(f"📞Phone: {business.get('phone', 'N/A')}")  # Show restaurant phone number
                    st.write(f"📍 Distance from your location: {business.get('distance_km', '?')} km")  # Show restaurant distance from city center
                    # Display opening hours
                    if business.get("opening_hours"):
                        with st.expander("🕒 Show opening hours"):
                            for day in business["opening_hours"]:
                                st.write(f"• {day}")
                    if business.get("is_open_now") is True:
                        st.write("🟢 **Open now**")  # Display 🟢 if the restaurant is currently open
                    elif business.get("is_open_now") is False:
                        st.write("🔴 **Currently closed**")  # Display 🔴 if the restaurant is currently closed
                    if business.get("website"):
                            st.markdown(f"[🌐 Website]({business['website']})", unsafe_allow_html=True)
    ...
    # "Surprise Me" button second (moved down)
